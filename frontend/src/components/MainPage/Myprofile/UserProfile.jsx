...
export default function UserProfile() {
  ...
  const [reviewToPeopleTotalPages, setReviewToPeopleToTalPages] =
    useState(null);
  ...
  const axios_ReviewToPeople = async () => {
    try {
      const response = await api.post(`/reviews/myWrite`, {
        memberId: id,
        role: 'USER',
        offset: 0,
        limit: 3,
  ...
  const axios_ReviewToMe = async () => {
    try {
      const response = await api.post(`/reviews/myReceive`, {
        memberId: id,
        role: 'USER',
        offset: 0,
        limit: 3,
  ...
  const closeModal = () => [setopenRecommendModal(false)];
  return (
    <>
      <div className="d-flex col-8 gap-5 align-items-start p-3">
        {myprofile && sellList && buyList && helpMeList && helpYouList && (
        ...
        <div className="d-flex col-8 flex-column " style={{ height: '100%' }}>
          ...
          {changeButton === '받은 후기' && (
            <ReviewToMeView
              id={id}
              renew={renew}
              setReviewToMeList={setReviewToMeList}
              reviewToMeList={reviewToMeList}
              reviewToMeTotalPages={reviewToMeTotalPages}
            />
          )}
          {changeButton === '보낸 후기' && (
            <ReviewToPeopleView
              id={id}
              setReviewToPeopleList={setReviewToPeopleList}
              reviewToPeopleList={reviewToPeopleList}
              reviewToPeopleTotalPages={reviewToPeopleTotalPages}
            />
          )}
        </div>
      </div>
