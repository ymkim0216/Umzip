...
export default function RecommendModalComponent({ companyId }) {
    ...
    const experienceDate = new Date(serviceInfo.experience);
    ...
    const hoverContent = (
        <div style={{ position: "absolute", top: "100%", left: "50%", transform: "translateX(-50%)", backgroundColor: "rgba(0, 0, 0, 0.8)", color: "white", padding: "0.5rem", borderRadius: "5px", display: "none" }}>
            Hover Content
        </div>
    );
    return (
        <>
            {serviceInfo && <div className="modal-dialog modal-dialog-scrollable">
                <div className="p-3 mt-4 gap-2 d-flex flex-column align-items-center" style={{ width: "100%", borderBottom: "1px solid #888888" }} >
                    ...
                    <div className="d-flex align-items-center gap-1 " style={{ position: "relative" }}> <motion.img style={{ width: "3rem", height: "3rem" }} src={imgsrc} alt="Random Image" /><h5 className="m-0">{serviceInfo.name}</h5>{hoverContent}</div>
